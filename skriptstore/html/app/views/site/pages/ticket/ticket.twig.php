...
<div class="complete" style="width:<?= 100 / $ticket->places * $placesnow ?>%"></div>
  ...
  <?php foreach (range(1, $ticket->places) as $i) { ?>
  <?php if ($places[$i]) { ?>
  <div class="one-place" id="game-place-<?= $i ?>">
<a href="/profile/<?= $places[$i]['user'] ?>" target="_blank"><img src="<?= $this->helper->getUserInfo($places[$i]['user'])->photo ?>"></a>
</div>
  <?php } else { ?>
...
<div class="b1">
<a href="/profile/<?= $ticket->winner_id ?>"><img src="<?= $this->helper->getUserInfo($ticket->winner_id)->photo ?>" alt="<?= $this->helper->getUserInfo($ticket->winner_id)->name ?>"></a>
</div>
<div class="b2">
<a href="/profile/<?= $ticket->winner_id ?>"><?= $this->helper->getUserInfo($ticket->winner_id)->name ?></a><br>
...
<span class="date"><?= $this->helper->unixToString('%d %b %Y, %H:%M', $places[$i]['time']) ?></span> <a href="/profile/<?= $places[$i]['user'] ?>" target="_blank"><?= $this->helper->getUserInfo($places[$i]['user'])->name ?></a> занял место <span class="ticket">#<?= $places[$i]['place'] ?></span>
</div>
<?php } ?>
<?php } ?>
