...
<div class="wrapper">
    ...
    <div class="case-page">
        ...
        <div class="spin">
            <div class="name">
                <h1><?= $case->name ?></h1>
                <div class="desc">Содержит от <b><?= $this->helper->getMinMaxCase($case->id)->min ?>р</b> до <b><?= $this->helper->getMinMaxCase($case->id)->max ?>р</b></div>

                <?php if ($case->type == 2) { ?>
                    <div class="digital">
                        <span>+ <?= $this->helper->getDigitalItemInfo($case->id)->name ?> <?= $this->helper->getDigitalItemInfo($case->id)->cost ?>р</span>
                    </div>
                <?php } elseif ($case->type == 3) { ?>
                    <div class="digital">
                        <span>+ <?= $this->helper->getDigitalItemInfo($case->id)->name ?></span>
                    </div>
                <?php } ?>

                <div class="payed">выдано <?= $this->helper->getBankCase($case->id) ?>р</div>
            </div>
            <div class="spin-line"></div>
            <div class="spin-inner">
                <div class="roulette">
                    <?php foreach ($items_reverse as $item) { ?>
                        <img src="<?= $this->helper->getItemInfo($item)->image ?>" alt="<?= $this->helper->getItemInfo($item)->name ?>" id="gift-id-<?= $this->helper->getItemInfo($item)->id ?>">
                    <?php } ?>
                </div>
            </div>

                <button class="btn blue rounded" onclick="Functions.OpenBonus(<?= $case->id ?>, this);" >Принять участие!</button>
            </div>
            <div class="button">
                <script>
        ...
        <div class="history-cases MarginTop-40">
            <?php foreach ($items as $item) { ?>
                <div class="history-case">
                    <div class="coin gold">
                        <img src="<?= $this->helper->getItemInfo($item)->image ?>" alt="<?= $this->helper->getItemInfo($item)->name ?>">
                    </div>
                </div>
            <?php } ?>
        </div>
